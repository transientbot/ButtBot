...
(function() {
	...
	if ($.changed == true && $.changed != null && $.changed != undefined && !$.inidb.exists('updates', 'installedNewBot') && $.inidb.get('updates', 'installedNewBot') != 'true') {
		...
		'installed2.3.6ss', 'installed2.3.6b', 'installedv2.3.7', 'installedv2.3.7b', 'installedv2.3.9', 'installedv2.3.9.1', 'installedv2.3.9.2'];
		for (i in versions) {
	...
	/* version 2.3.9.2 updates */
	if (!$.inidb.exists('updates', 'installedv2.3.9.2') || $.inidb.get('updates', 'installedv2.3.9.2') != 'true') {
		$.consoleLn('Starting PhantomBot update 2.3.9.2 updates...');

		$.consoleLn('Setting up new default custom commands...');
		$.inidb.set('queue', 'interval', '10');
		$.inidb.set('queue', 'noticeToggle', 'true');

		$.consoleLn('PhantomBot update 2.3.9.1 completed!');
		$.inidb.set('updates', 'installedv2.3.9.2', 'true');
	}
